...
async function main() {
  ...
  console.log(`\n# Investment Pool with 82 tokens`);

  await multihop((index: number) => getWeightedPool(vault, tokens, 82, index), false);
  await multihop((index: number) => getWeightedPool(vault, tokens, 82, index), true);
